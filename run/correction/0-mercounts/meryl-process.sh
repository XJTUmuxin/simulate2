...
if [ -e ./test.ms16.histogram -a -e ./test.ms16.dump -a -e ./test.ms16.ignore.gz ] ; then
  exit 0
fi
...
if [ -e test.ms16.histogram ]; then
  exists1=true
else
  exists1=false
fi

if [ -e test.ms16.dump ]; then
  exists2=true
else
  exists2=false
fi

if [ -e test.ms16.ignore.gz ]; then
  exists3=true
else
  exists3=false
fi
if [ $exists1 = true -a $exists2 = true -a $exists3 = true ] ; then
  echo "Output files 'test.ms16.histogram', 'test.ms16.dump' and 'test.ms16.ignore.gz' exist in 'correction/0-mercounts'."
  exit 0
fi
...
if [ ! -e ./test.ms16/merylIndex ] ; then
  /home/zmx/work/canu/build/bin/meryl threads=1 memory=2 \
    greater-than 1 \
      output test.ms16.WORKING \
      union-sum  \
        ./test.01.meryl \
  && \
  mv -f ./test.ms16.WORKING ./test.ms16
  ...
  rm -rf ./test.01.meryl ./test.01.meryl.err
fi
...
if [ ! -e ./test.ms16.histogram ] ; then
  /home/zmx/work/canu/build/bin/meryl threads=1 memory=1 \
    statistics ./test.ms16 \
  > ./test.ms16.histogram.WORKING \
  && \
  mv ./test.ms16.histogram.WORKING ./test.ms16.histogram
fi
...
if [ ! -e ./test.ms16.dump ] ; then
  /home/zmx/work/canu/build/bin/meryl threads=1 memory=2 \
    print ./test.ms16.##.dump \
      at-least threshold=50 \
      at-least word-frequency=0.0000001 \
        ./test.ms16

  cat ./test.ms16.??.dump > ./test.ms16.dump
  rm -f ./test.ms16.??.dump
fi
...
if [ ! -e ./test.ms16.ignore.gz ] ; then
  numKmers=`wc -l < ./test.ms16.dump`
  totKmers=`/home/zmx/work/canu/build/bin/meryl statistics ./test.ms16 | grep present | awk '{ print $2 }'`


  ./meryl-make-ignore.pl $numKmers $totKmers < ./test.ms16.dump | gzip -1c > ./test.ms16.ignore.gz
fi
