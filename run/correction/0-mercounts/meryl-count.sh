...
if [ -e ./test.$jobid.meryl/merylIndex ] ; then
...
if [ -e test.$jobid.meryl.tar.gz ]; then
  exist1=true
else
  exist1=false
fi
...
/home/zmx/work/canu/build/bin/meryl k=16 threads=1 memory=2 \
  count \
    segment=$jobid/01 ../../test.seqStore \
    output ./test.$jobid.meryl.WORKING \
&& \
mv -f ./test.$jobid.meryl.WORKING ./test.$jobid.meryl
