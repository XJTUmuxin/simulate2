...
#  This script should be executed from correction/test.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./test.ovlStore.BUILDING/$jobname.info -a ! -e ./test.ovlStore.BUILDING/$jobname.started ] ; then
  echo "Sorting job finished; info file './test.ovlStore.BUILDING/$jobname.info' exists."
  exit
fi
#
#  Sort!
...
$bin/ovStoreSorter \
  -deletelate \
  -O  ./test.ovlStore.BUILDING \
  -S ../test.seqStore \
  -C  ./test.ovlStore.config \
  -f \
  -s $jobid \
  -M 1 
